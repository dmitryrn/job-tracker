...
func TestDiscoverySettingsAPI(t *testing.T) {
	...
	require.Len(t, result.Settings.LinkedIn, 2)
	assert.False(t, result.Settings.LinkedIn[0].Enabled)
	assert.Equal(t, "Serbia", result.Settings.LinkedIn[1].Location)

	response = requestWithBody(handler, http.MethodPut, "/api/discovery-settings", `{"adzuna":{"enabled":false,"query":"platform engineer","country":"de","maxDaysOld":14,"maxPages":2,"resultsPerPage":25,"workplace":"remote"},"remotive":{"enabled":true,"query":"platform engineer","category":"software-development"},"jobicy":{"enabled":true,"count":25,"geo":"europe","industry":"engineering","tag":"golang"},"linkedin":[{"id":1,"name":"Europe","enabled":false,"sortOrder":1,"query":"platform engineer","location":"Europe","postedWithin":"r604800","workplace":"2","experienceLevel":"4","limit":1000},{"id":2,"name":"Serbia","enabled":true,"sortOrder":2,"query":"platform engineer","location":"Serbia","postedWithin":"r604800","workplace":"2","experienceLevel":"4","limit":1000}]}`)
	require.Equal(t, http.StatusOK, response.Code)
	require.NoError(t, json.NewDecoder(response.Body).Decode(&result))
	...
	assert.Equal(t, "Europe", result.Settings.LinkedIn[0].Location)
	assert.Equal(t, "r604800", result.Settings.LinkedIn[0].PostedWithin)
	assert.Equal(t, "2", result.Settings.LinkedIn[0].Workplace)
	assert.Equal(t, "4", result.Settings.LinkedIn[0].ExperienceLevel)
	assert.Equal(t, 1000, result.Settings.LinkedIn[0].Limit)

	response = requestWithBody(handler, http.MethodPut, "/api/discovery-settings", `{"adzuna":{"enabled":false,"query":"","country":"de","maxDaysOld":14,"maxPages":2,"resultsPerPage":25,"workplace":"remote"},"remotive":{"enabled":true,"query":"platform engineer","category":"software-development"},"jobicy":{"enabled":true,"count":25,"geo":"europe","industry":"engineering","tag":"golang"},"linkedin":[{"id":1,"name":"Europe","enabled":false,"sortOrder":1,"query":"platform engineer","location":"Europe","postedWithin":"r604800","workplace":"2","experienceLevel":"4","limit":25}]}`)
	assert.Equal(t, http.StatusBadRequest, response.Code)

	response = requestWithBody(handler, http.MethodPut, "/api/discovery-settings", `{"adzuna":{"enabled":false,"query":"platform engineer","country":"de","maxDaysOld":14,"maxPages":2,"resultsPerPage":25,"workplace":"remote"},"remotive":{"enabled":true,"query":"platform engineer","category":"software-development"},"jobicy":{"enabled":true,"count":25,"geo":"europe","industry":"engineering","tag":"golang"},"linkedin":[{"id":1,"name":"Europe","enabled":false,"sortOrder":1,"query":"platform engineer","location":"Europe","postedWithin":"r604800","workplace":"2","experienceLevel":"4","limit":1001}]}`)
	assert.Equal(t, http.StatusBadRequest, response.Code)
}
