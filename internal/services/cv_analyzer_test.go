...
func TestCVAnalyzerReturnsVersionedEvidenceBackedProfile(t *testing.T) {
	analyzer := NewCVAnalyzer(fakeCVCompletionClient{response: openrouter.ChatResponse{
		...
		Content: `{
			...
			"experience":[{"company":"RelayFox GmbH","title":"Senior Backend Engineer","startDate":"May 2022","endDate":"Present","evidence":["Designed and operated Go services using PostgreSQL, Kafka, Redis, Docker, and Kubernetes."]}],
			"skills":[{"name":"Go","evidence":["Designed and operated Go services using PostgreSQL, Kafka, Redis, Docker, and Kubernetes."]}],
...
func TestCVAnalyzerRejectsClaimsWithoutEvidence(t *testing.T) {
	analyzer := NewCVAnalyzer(fakeCVCompletionClient{response: openrouter.ChatResponse{
		Content: `{"name":"","headline":"","location":"","workAuthorization":[],"experience":[],"skills":[{"name":"Go","evidence":[]}],"constraints":[],"unknowns":[]}`,
	}})
