...
func TestLLMProfileJobMatcherReturnsValidatedAssessment(t *testing.T) {
	...
	assessment, err := NewLLMProfileJobMatcher(client).Match(context.Background(),
	...
	assert.Equal(t, ProfileJobMatcherModel, client.request.Model)
...
func TestLLMProfileJobMatcherRejectsInvalidScore(t *testing.T) {
	...
	_, err := NewLLMProfileJobMatcher(client).Match(context.Background(), models.BrowseJob{}, models.JobAnalysisRecord{}, models.UserProfile{})
